...
![](/assets/images/posts/threathunting101-image13.png)
...
These events confirm our hypothesis that this host likely has a dropper on it. Fortunately, we have the hashes and the names of the file from the email. Let’s see if we can find it on the system. We will issue a simple query of the string name to demonstrate how we could find it. Typically, we would use hashes, but sometimes naming also works.

![](/assets/images/posts/threathunting101-image14.png)
...
Now, we have two paths for this hunt, as we have two leads. The first option would be to check for any interesting files being written to the `Temp` folders of our hosts recently. The other would be to check for any new, interesting registry keys in `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`.
